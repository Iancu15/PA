...
// am pus value double pentru ca o sa il inlocuiesc
// cu value/weight
...
// cum compar double, iar functia intoarce int
// a trebuit sa folosesc if in loc de o simpla scadere a - b
int compareProducts(const void * a, const void * b)
{
	double valueA = (*(Product*)a).value;
	double valueB = (*(Product*)b).value;
	// pentru a ordona descrescator returnez -1
	// pentru cazul in care valueA > valueB
	if (valueA - valueB > 0.0) {
		return -1;
	} else if (valueA - valueB < 0.0) {
		return 1;
	} else {
		return 0;
	}
...
double fractionalKnapsack(Product *arr, int n, int Gmax) {
	// calculez cat de valoroasa e unitatea de masa a fiecarui obiect
	for (int i = 0; i < n; i++) {
		arr[i].value /= arr[i].weight;
	}

	// ordonez descrescator produsele dupa valoarea pe unitatea de masa
	qsort(arr, n, sizeof(Product), compareProducts);
	int spaceLeft = Gmax;
	double sumValue = 0;
	for (int i = 0; i < n; i++) {
		// daca nu incape tot obiectul, pun o fractie din el
		// cat sa ocupa tot spatiul ramas liber si ies din for
		if (spaceLeft - arr[i].weight < 0) {
			sumValue += arr[i].value * spaceLeft;
			break;
		}

		// daca s-a ajuns aici inseamna ca mai pot adauga
		// obiecte intregi
		sumValue += arr[i].value * arr[i].weight;
		spaceLeft -= arr[i].weight;
	}
...
// compar crescator dupa momentul in care se termina show-ul
int compareShows(const void * a, const void * b)
{
	return (*(Show*)a).end - (*(Show*)b).end;
}

int *planningShows(Show *arr, int n, int *size) {
	// ordonez crescator dupa momentul in care se termina show-ul
	qsort(arr, n, sizeof(Show), compareShows);

	int* availableShows = (int*) malloc(n * sizeof(int));
	int availableShowsidx = 0;

	// endtime-urile sunt pozitive, asa ca pot sa-l setez -1
	int occupiedEndTime = -1;
	for (int i = 0; i < n; i++) {
		// daca inceputul spectacolului este dupa terminarea
		// ultimului spectacol adaugat in vector, atunci il
		// pot adauga si pe el
		if (arr[i].begin >= occupiedEndTime) {
			occupiedEndTime = arr[i].end;
			availableShows[availableShowsidx] = arr[i].idx;
			availableShowsidx++;
		}
	}

	*size = availableShowsidx;
